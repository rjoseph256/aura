...
`StatPair` (`Aura/Sources/Theme/StatPair.swift`) gains two optional properties for this call —
`valueLineLimit: Int?` (default `nil`) and `valueMinimumScaleFactor: CGFloat` (default `1`) —
applied only to its value `Text`, never its label. Both default to the prior unconstrained
behavior, so `RideSummaryView` and `ShareCardView` (`StatPair`'s other callers) are unaffected.
