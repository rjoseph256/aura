...
- [x] **Step 2:** Tests must cover: absent → nil; the null case → nil; a present value → non-nil; **a round-trip** — encode a real `Route`, map it through the boundary, decode it back, assert equal; **and a wide integer from raw wire bytes**.

  *Corrected during execution.* This plan claimed the round trip was "the only assertion that can catch a lossy reduction", naming the `.integer`/`.double` collapse. **That is false, and the implementer disproved it with a mutation:** every number in a `Route` is already a `Double`, so the round trip passes whether or not the two cases are collapsed. A round-trip-only suite would have shipped exactly the defect this step warned about. The round trip earns its place on a *different* failure — flattening the object case — which the integer test misses. Two tests, two failure modes.
